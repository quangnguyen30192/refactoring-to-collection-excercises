...
{
    private function emailTo(Collection $employees, string $customer)
    {
        /*
        ...
        return  $employees->filter(function ($employee) use ($customer) {
            return collect($employee['sales'])->contains('customer', $customer); // or pluck('customer')->contain($customer)
        })->implode('email', ',');
    }
